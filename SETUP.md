...
This guide will help you set up the Testomatic Device Management Dashboard.
...
You should see `(venv)` in your terminal prompt. Keep this terminal open and the virtual environment activated for all subsequent steps.
...
Then edit `conf/local_settings.py` and keep only the SQLite configuration (the first section, lines 1-28). Remove or comment out the MySQL section below.
...
This will create the SQLite database file (`db.sqlite3`) and set up all the database tables.
...
Follow the prompts to create an admin account. You'll use this to log into the admin interface.
