...
- **[models.py](pyproj/erp/models.py)**:
  ...
  - `BomEquivalenceRule` — a rule applied during BOM CSV import that remaps a (device, package, value) triple to a different one before the duplicate check. Fields: `library`, `from_device`/`to_device`, `from_package`/`to_package`, `from_value`/`to_value` — "from" fields blank to match any value, "to" fields blank to leave that part of the triple unchanged. Field order on the model, form, and templates groups each From/To pair adjacently (Device pair, then Package pair, then Value pair) rather than all-from-then-all-to, and the merged settings page draws a header grouping + divider (table) or a bordered `<fieldset>` (edit page) around each pair. `Meta.ordering = ['library', 'from_device', 'from_package', 'from_value']`.
